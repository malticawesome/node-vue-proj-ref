  ...
  actions: {
    async doInit({ commit }, id) {
      try {
        ...
        if (id) {
          record = await LogService.find(id);
        }
    ...
    async doCreate({ commit }, values) {
      try {
        ...
        await LogService.create(values);
        commit('CREATE_SUCCESS');
        Message.success(
          i18n('entities.log.create.success'),
        );
        routerAsync().push('/log');
      } catch (error) {
        Errors.handle(error);
    ...
    async doUpdate({ commit }, { id, values }) {
      try {
        ...
        await LogService.update(id, values);

        commit('UPDATE_SUCCESS');
        Message.success(
          i18n('entities.log.update.success'),
        );
        routerAsync().push('/log');
      } catch (error) {
        Errors.handle(error);
