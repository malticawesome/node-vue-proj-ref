...
import logListExporterFields from '@/modules/log/log-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (
          !logListExporterFields ||
          !logListExporterFields.length
        ) {
          throw new Error(
            'logListExporterFields is required',
          );
        }
