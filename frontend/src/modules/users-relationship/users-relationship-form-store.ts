  ...
  actions: {
    async doInit({ commit }, id) {
      try {
        ...
        if (id) {
          record = await UsersRelationshipService.find(id);
        }
    ...
    async doCreate({ commit }, values) {
      try {
        ...
        await UsersRelationshipService.create(values);
        commit('CREATE_SUCCESS');
        Message.success(
          i18n('entities.usersRelationship.create.success'),
        );
        routerAsync().push('/users-relationship');
      } catch (error) {
        Errors.handle(error);
    ...
    async doUpdate({ commit }, { id, values }) {
      try {
        ...
        await UsersRelationshipService.update(id, values);

        commit('UPDATE_SUCCESS');
        Message.success(
          i18n('entities.usersRelationship.update.success'),
        );
        routerAsync().push('/users-relationship');
      } catch (error) {
        Errors.handle(error);
